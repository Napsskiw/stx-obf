...
const log = (message, error = null) => {
    ...
    const prefix = `\x1b[36m[ Sniith Obfuscated ]\x1b[0m`;
...
bot.start(async (ctx) => {
    ...
    const inlineKeyboard = [
        [
            { text: '𝑪𝒉𝒂𝒏𝒏𝒆𝒍', url: 'https://t.me/stxpos' },
            { text: '𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓', url: 'https://t.me/Sniith' }
        ],
        [
    ...
    setTimeout(async () => {
        await ctx.replyWithPhoto("https://ibb.co.com/j9wLJg1W", {
            caption: `
✦ 𝒀𝒐𝒌𝒐𝒔𝒐, *${username}* ✦  
お役に立てれば幸いです 💋  
━━━━━━━━━━━━━━━━━━━━━  
...
bot.action("start_menu", (ctx) => {
    ...
    ctx.editMessageMedia(
        {
            type: 'photo',
            media: "https://ibb.co.com/j9wLJg1W", // link catbox
            caption: `
✦ 𝒀𝒐𝒌𝒐𝒔𝒐, *${username}* ✦  
お役に立てれば幸いです 💋  
━━━━━━━━━━━━━━━━━━━━━  
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            "❌ E̶R̶R̶O̶R̶ │ ⓘ ᴀɴᴅᴀ ʜᴀʀᴜs ʙᴇʀɢᴀʙᴜɴɢ ᴋᴇ ᴄʜᴀɴɴᴇʟ ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ sᴇʙᴇʟᴜᴍ ᴍᴇᴍᴀᴋᴀɪ ʙᴏᴛ ᴏʙғ ɴʏᴀ!! \n" +
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://https://t.me/stxpos)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: "✅ *File terenkripsi siap!*\nSUKSES ENCRYPT ©️ Sniith 🕊", parse_mode: "Markdown" }
        );
...
// Command /violence  (diperkuat dengan pemeriksaan channel)
bot.command("violence ", async (ctx) => {
users.add(ctx.from.id);
    saveUsers(users);
    const isMember = await checkChannelMembership(ctx);
    if (!isMember) {
        return ctx.replyWithMarkdown(
            "❌ E̶R̶R̶O̶R̶ │ ⓘ ᴀɴᴅᴀ ʜᴀʀᴜs ʙᴇʀɢᴀʙᴜɴɢ ᴄʜᴀɴɴᴇʟ ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ ‼️ \n" +
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://https://t.me/stxpos)"
        );
    }

    if (!ctx.message.reply_to_message || !ctx.message.reply_to_message.document) {
        return ctx.replyWithMarkdown("❌ *Error:* Balas file .js dengan `/violence  [level]`!");
    }
    ...
    try {
        ...
        await ctx.replyWithMarkdown(
            ...
            `_Level: ${encryptionLevel} | Powered ©️ Sniith`
        );
        await ctx.replyWithDocument(
            { source: encryptedPath, filename: `eval-encrypted-${file.file_name}` },
            { caption: "✅ *File terenkripsi siap!*\n_SUKSES ENCRYPT ©️ Sniith 🕊", parse_mode: "Markdown" }
        );
...
// Command /encmega <size_in_mb> (Obfuscation dengan ukuran besar)
bot.command("encmega", async (ctx) => {
users.add(ctx.from.id);
    saveUsers(users);
    const isMember = await checkChannelMembership(ctx);
    if (!isMember) {
    ...
    if (args.length < 2 || !args[1] || isNaN(args[1])) {
        return ctx.replyWithMarkdown("❌ *Error:* Gunakan format `/encmega <size_in_mb>` dengan ukuran dalam MB (angka)!");
    }
    const targetSizeMB = Math.max(1, parseInt(args[1], 10)); // Minimal 1 MB

    if (!ctx.message.reply_to_message || !ctx.message.reply_to_message.document) {
        return ctx.replyWithMarkdown("❌ *Error:* Balas file .js dengan `/encmega <size_in_mb>`!");
    }
...
// Command /encintens  <intensity> (Obfuscation dengan metode Max dan intensitas yang dapat diatur)
bot.command("encintens ", async (ctx) => {
users.add(ctx.from.id);
    saveUsers(users);
    const isMember = await checkChannelMembership(ctx);
    if (!isMember) {
    ...
    const args = ctx.message.text.split(" ");
    if (args.length < 2 || !args[1] || isNaN(args[1])) {
        return ctx.replyWithMarkdown("❌ *Error:* Gunakan format `/encintens  <intensity>` dengan intensitas (1-10)!");
    }
    const intensity = Math.min(Math.max(1, parseInt(args[1], 10)), 10); // Batas 1-10

    if (!ctx.message.reply_to_message || !ctx.message.reply_to_message.document) {
        return ctx.replyWithMarkdown("❌ *Error:* Balas file .js dengan `/encintens  <intensity>`!");
    }
...
// Jalankan bot
bot.launch(() => log("Encrypt Bot ©️ Sniith berjalan..."));
process.on("unhandledRejection", (reason) => log("Unhandled Rejection", reason));

bot.command('payment', (ctx) => {
    ctx.replyWithPhoto(
        'https://files.catbox.moe', // Foto QRIS
        {
            caption: 'Silakan scan QRIS untuk pembayaran atau klik tombol di bawah untuk nomor e-wallet dan sertakan bukti berupa ss ke @Sniith:',
            reply_markup: {
                inline_keyboard: [
...
const paymentNumber = '083193340723';
